    ...
    const onDrop = useCallback((acceptedFiles: File[], rejectedFiles: any[]) => {
        ...
        if (rejectedFiles.length > 0) {
            ...
            if (rejection.errors[0]?.code === 'file-too-large') {
                setErrorMessage('Datei ist zu groß. Maximale Größe: 10MB');
            } else if (rejection.errors[0]?.code === 'file-invalid-type') {
                setErrorMessage('Nur Bilddateien sind erlaubt (JPG, PNG, GIF, WEBP)');
            } else {
    ...
    const uploadFile = async (file: File) => {
        ...
        try {
            ...
            if (response.status >= 200 && response.status < 300) {
                ...
                const errorMsg = `Fehler beim Hochladen: ${response.status} ${response.statusText}`;
                    ...
                    <div className="dropzone-content">
                        {isDragActive ? (
                            ...
                            <div className="upload-message">
                                ...
                                <p><strong>Bild auswählen</strong> oder hier hineinziehen</p>
                                <p className="upload-hint">Klicken Sie, um eine Datei auszuwählen, oder ziehen Sie eine Datei hierher</p>
                            </div>
                        )}
                    </div>
                )}
